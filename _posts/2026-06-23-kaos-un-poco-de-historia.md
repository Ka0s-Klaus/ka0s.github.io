...
```mermaid
flowchart TD
    A[🏛️ Antigüedad\n700 a.C. – 400 d.C.\nMitos y Autómatas] --> B[⚙️ Edad Media\nSiglos XIII–XVII\nMáquinas Mecánicas]
    B --> C[📐 Siglo XIX\nLógica y Álgebra\nBabbage, Boole]
    C --> D[💡 1936–1950\nTuring y los Fundamentos]
    ...
    K --> L[🏆 2012–2017\nImageNet, AlphaGo, Transformer]
    L --> M[💬 2017–2022\nGPT, BERT, LLMs]
    M --> N[🌍 Nov 2022\nChatGPT: IA para todos]
    N --> O[⚡ 2023–2024\nGPT-4, Claude, Gemini\nCarrera Frontera]
    O --> P[🤖 2025–2026\nEra de los Agentes Autónomos]
```
...
```mermaid
flowchart LR
    T[Juez Humano] -->|Texto| A[¿Humano o Máquina?]
    H[Humano Real] -->|Respuestas| T
    M[Máquina Candidata] -->|Respuestas| T
    A -->|Si no distingue| R[✅ La máquina supera el Test]
    A -->|Si detecta la máquina| F[❌ No supera el Test]
    style R fill:#2d6a4f,color:#fff
    style F fill:#6b2737,color:#fff
```
...
```mermaid
flowchart LR
    DB[♟️ Deep Blue 1997\nAjedrez: Máquina vence al hombre]
    WA[🎙️ Watson 2011\nJeopardy: IA supera al humano\nen lenguaje natural]
    AN[🧠 AlexNet 2012\nImageNet: Revolución\ndel Deep Learning]
    DB --> WA --> AN
    style DB fill:#1a1a2e,color:#e0e0ff
    style WA fill:#16213e,color:#e0e0ff
    style AN fill:#0f3460,color:#e0e0ff
```
...
```mermaid
flowchart TD
    subgraph "Factores que hicieron posible el Deep Learning"
        D[📊 Datos Masivos\nInternet + Digitalización]
        G[⚡ GPUs\nNVIDIA CUDA 2007]
        A[🧮 Algoritmos\nBackpropagation mejorado]
        D & G & A --> DL[🧠 Deep Learning\nFunciona de verdad]
    end
```
...
```mermaid
flowchart LR
    subgraph "El Tsunami de Adopción"
        A[📅 30 Nov 2022\nLanzamiento ChatGPT] -->|5 días| B[👥 1M usuarios]
        B -->|2 meses| C[👥 100M usuarios]
        C -->|1 año| D[📊 180M usuarios\napr. 2024]
    end
```
...
```mermaid
flowchart TD
    subgraph "Era Generativa 2022–2024"
        U[Usuario] -->|Pregunta| M[Modelo LLM]
        M -->|Respuesta texto| U
    end
    subgraph "Era Agéntica 2025–"
        UA[Usuario] -->|Objetivo de alto nivel| AG[Agente IA]
        AG -->|Planifica| P[Plan de tareas]
        P -->|Ejecuta| T1[🌐 Navega web]
        P -->|Ejecuta| T2[💻 Escribe y ejecuta código]
        P -->|Ejecuta| T3[📡 Llama APIs externas]
        P -->|Ejecuta| T4[📁 Gestiona archivos]
        T1 & T2 & T3 & T4 -->|Resultado| UA
    end
...
```mermaid
graph LR
    subgraph "Ciclos Históricos de la IA"
        direction LR
        P1[🌞 1ª Primavera\n1956–1969\nOptimismo desbordante] --> I1[❄️ 1er Invierno\n1970–1979\nRecorte de fondos]
        I1 --> P2[🌞 2ª Primavera\n1980–1987\nSistemas Expertos]
        P2 --> I2[❄️ 2do Invierno\n1987–1993\nColapso mercado]
        I2 --> P3[🌦️ Renacimiento\n1997–2011\nDeep Blue, Watson]
        P3 --> P4[☀️ Revolución DL\n2012–2022\nAlphaGo, GPT-3]
        P4 --> P5[🔥 Era LLM\n2022–hoy\nChatGPT, Claude, AGI?]
    end
```
